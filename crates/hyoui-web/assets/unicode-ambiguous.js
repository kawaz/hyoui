// hyoui web — East Asian Ambiguous を width 2 として扱う wcwidth provider (?ambw=full)。
//
// 「Ambiguous」は EastAsianWidth.txt で `A` に分類される文字。CJK 文脈では全角、
// それ以外では半角で組まれてきた歴史があり、どちらで数えるかは端末の設定に委ねられて
// いる (tmux の `utf8-ambiguous-width`、iTerm2 の "Treat ambiguous-width characters
// as double-width" と同じ選択)。既定は half (= 素の unicode11 幅)。
//
// **これは opt-in であり、既定にはしない。** daemon 側の vt100 と TUI アプリ本体は
// Ambiguous を幅 1 として数える (実測で一致を確認済み)。web だけ 2 にすると、
// screen dump が渡してくる行を xterm.js が自前の幅で組み直す際に、対象文字より後ろが
// 1 文字あたり 1 桁ずつ右へずれる (行頭で再同期するのでずれは行内に閉じる)。
// screen state を正本とする設計 (DR-0013 / DR-0005) からの意図的な乖離になるため、
// 明示指定した閲覧者だけが踏む経路にしてある。
//
// 影響範囲は装飾記号だけではない。**box drawing (U+2500-254B) と block elements
// (U+2580-258F) も Ambiguous** なので、full を選ぶと TUI の罫線・ブロック罫線が
// 2 セル幅になり、枠線を引くアプリの表示は大きく崩れる。日本語混在の文書を読む用途で
// 幅を揃えたい場合に使い、TUI を操作する用途では half のままにするのが実用的。
//
// テーブルの出典と再生成:
//   Unicode 16.0.0 の EastAsianWidth.txt から `; A` の行を集め、隣接レンジを
//   マージしたもの。179 レンジ / 138739 codepoint (PUA plane 15/16 を含むため
//   codepoint 数は大きいが、レンジ数は少ない)。
//   https://www.unicode.org/Public/16.0.0/ucd/EastAsianWidth.txt
//
//   for line in open("EastAsianWidth.txt"):
//       line = line.split("#")[0].strip()
//       if not line: continue
//       rng, cls = [x.strip() for x in line.split(";")]
//       if cls == "A": ...  # rng は "XXXX" か "XXXX..YYYY"
(() => {
  // ソート済み・非重複の [開始, 終了] 閉区間 (両端含む)。
  const RANGES = [
  ...
  window.HyouiAmbiguousWidth = {
    isAmbiguous,

    /// unicode11 addon が持つ UnicodeV11 provider を取り出す。
    ///
    /// UMD bundle が公開しているのは Unicode11Addon クラスだけで UnicodeV11 は
    /// 内部モジュールに閉じている。一方 ITerminalAddon の `activate(terminal)` は
    /// 「terminal.unicode.register(provider)」を呼ぶだけと決まっているので、
    /// register を捕まえるスタブを渡せば provider を取得できる。xterm.js の内部
    /// (`_core.unicodeService._providers`) を覗かずに済む分、vendor 差し替えに強い。
    captureUnicode11() {
      if (!window.Unicode11Addon) return null;
      let captured = null;
      new window.Unicode11Addon.Unicode11Addon().activate({
        unicode: { register(provider) { captured = provider; } },
      });
      return captured;
    },

    /// base provider に委譲しつつ、Ambiguous な文字だけ幅 2 を返す provider を作る。
    ///
    /// 幅 0 の文字は 0 のまま通す。Ambiguous には結合文字 (U+0300-036F)、
    /// variation selector (U+FE00-FE0F / U+E0100-E01EF)、Nerd Font が使う PUA が
    /// 含まれており、これらを 2 に上げるとカーソル位置と Powerline 記号が壊れる。
    /// 幅 2 の文字も base の値をそのまま返す (Ambiguous と Wide は排他なので実際には
    /// 該当しないが、判定を base 側に委ねておく)。
    ///
    /// この vendor (xterm.js 5.3.0) の UnicodeService が provider に要求するのは
    /// version と wcwidth だけ。
    wrapProvider(base, version) {
      return {
        version,
        wcwidth(cp) {
          const w = base.wcwidth(cp);
          return w === 1 && isAmbiguous(cp) ? 2 : w;
        },
      };
    },
  };
