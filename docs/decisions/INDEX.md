...
| [DR-0025](./DR-0025-daemon-reducer-and-domain-formalization.md) | 📝 Draft (2026-06-30) | Daemon Reducer 化と全ドメイン event の形式化。`parallel_input_serialized_by_auto_lock` 試験で発見した race の構造的原因 (= lock state と PTY write の論理軸分離不在) を起点に、daemon 内部設計を「全 IO を message に統一 + reducer pure function + 直接触らない」方針に再構築。6 domain (Tty/Child/Serve/Client/Screen/Lock) reducer に分割、TTY は Layer 1 (raw bytes) → 2 (parsed) → 3 (semantic) の段階意味化 + 150-200 variant 規模 enum カタログ (規格名義務化、URL は補助)、Screen は ScreenWriteEvent (raw 通知) + WatchRegistration (region/matcher/flow の 3 軸直交、polling 不採用 = 仮想 screen 自前管理なので event-driven 十分) で構築。Child / Serve / Client は formal state machine 化。8 Phase migration plan、Open Questions 12 件 |
