...
| [DR-0024](./DR-0024-env-scrub-config-file.md) | ⬜ 未実装 (2026-06-22 起票、DR-0023 を redesign) | 子 PTY env scrub の config ファイル化と CLI flag 最小化。CLI を `--no-scrub-env` 1 個に絞り、`~/.config/hyoui/config.toml` の `[scrub_env_targets.<target>]` 階層で `inherit_builtin` + `kill_glob` + `keep_glob` を user 制御。target 推定は argv basename のみ (env wrapper unwrap なし)、`HYOUI_*` protected guard 維持、builtin 9 env (claude) 不変。hyoui 初の config ファイル機構 |
