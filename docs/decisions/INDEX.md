...
| [DR-0023](./DR-0023-child-env-scrub.md) | ✅ 実装済 (2026-06-21、target-aware env scrub 配線完了、claude 組み込み 9 env、`--no-scrub-env`/`--scrub-env-target`/`--scrub-env-add`/`--scrub-env-keep` 4 flag、HYOUI_* protected guard、実機マトリクス 7 case green) | 子 PTY env scrub (= 親 Internal Context env の漏洩防止)。`hyoui run -- claude` で親 Claude Code の `CLAUDE_CODE_SESSION_ID` 等が子 session に漏れて誤認させる問題 (issue #1) を target 推定 + builtin kill_glob + flag overrides + protected guard で根治。透過原則の例外として DR-0018 / DR-0020 と同枠で justify、削除のみ最小介入。新 protocol message 0 個 (`DaemonizeInit.scrub_env` field のみ追加) |
