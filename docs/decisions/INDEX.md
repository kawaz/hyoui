...
| [DR-0025](./DR-0025-tty-message-machine.md) | 📝 Draft (2026-06-30) | TTY Message Machine 化と TTY event の網羅 enum カタログ。`parallel_input_serialized_by_auto_lock` 試験で発見した race の構造的原因 (= lock state と PTY write の論理軸分離不在) を起点に、daemon 内部設計を「全 IO を message に統一 + reducer pure function + machine 以外は fd/state を直接触らない」方針に再構築。TTY 由来 event を 7 軸 (Tty/Ioctl/FlowCtl/Child/Signal/ClientReq/Timer) に分類、Layer 1 (raw bytes) → 2 (parsed) → 3 (semantic) の段階意味化、150-200 variant 規模の enum カタログを一次情報リンク義務化付き doc comment で網羅 (scoreboard 運用、AI 推測列挙との区別、規格出典追跡可能性確保) |
