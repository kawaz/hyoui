...
| 2026-07-03 | bug | wip | [bug-flaky-serve-tail-follow-tail-end](./2026-07-03-bug-flaky-serve-tail-follow-tail-end.md) | serve_tail_follow_receives_tail_end_on_child_exit の flaky — 子 exit 検出が client frame 処理を追い越して tail.request を捨てる race を**決定的再現で特定・修正**(regression test 追加)。ただし第 2 の原因が未解決 (macOS でも `daemon::session` 単位で 4/20 再現、単独実行は 30/30 green → daemon 多数並走時の global state 共有が最有力) |
