# cmux-msg 検証からのフィードバック (DR-0010 v0.2.0 scope 確定後の refresh 版)

- Status: Open
- Date: 2026-05-27
- Priority: Middle
- 発見元: kawaz/claude-cmux-msg main で hyoui 0.1.0 を試した検証セッション
- 経緯: 初版を [[DR-0010]] 確定前に書いて main 系統から orphan 化したため、v0.2.0 scope 確定後の現実に合わせて書き直したもの

## サマリ

claude-cmux-msg の動作確認文脈で hyoui 0.1.0 を一通り叩いた検証から得たフィードバック。基本動作 (run --mode=headless / list / status / tail / wait / kill / attach / completion) は期待通り動作。multi-client 並列接続も確認。

特筆: **`hyoui run --mode=headless` で claude を起動 → 初期プロンプトで `cmux-msg subscribe` を Monitor 起動させる** ことで、`hyoui input keys` (= [[2026-05-26-feature-claude-tui-automation]] で計画済) 無しでも cmux-msg メッセージング経路で child claude を完全に対話的に操作できることを実証した。並列 2 child + 並列 send/reply まで成立。

## v0.1.0 で残る bug (patch 候補)
...
v0.2.0 で `kill` subcommand の去就 (= 後述 B7) が決まると、socket cleanup 経路の設計も連動するはず。

## v0.2.0 scope に組み込みたい改善
...
### B6: 個別 `--help` の取りこぼし (再編時の予防的注意喚起)

0.1.0 では `hyoui list --help` / `hyoui kill --help` / `hyoui completion --help` の 3 つが root help にフォールバックする。`run/attach/status/tail/wait` は個別 help あり。
...
## 仕様確認したい点 (decisions/runbooks への反映が必要)
...
v0.2.0 着手前に決めたいが、本 DR (= DR-0010) では言及がないので別 DR or 本 issue で確認したい。

## cmux-msg + hyoui 連携で見えた使い方 (既存 issue との接続)
...
- [[../decisions/DR-0010]] — v0.2.0 scope re-scope (本 issue が前提とする scope)
- [[../decisions/DR-0005]] — 外側自動操作主軸の思想
- [[2026-05-26-feature-claude-tui-automation]] — `hyoui input keys` の本命 use case (本 issue と直接接続)
- [[../findings/2026-05-27-headless-claude-remote-control-leak]] — hyoui で headless 起動した claude も Remote Control の影響を受ける finding (別ファイル)
