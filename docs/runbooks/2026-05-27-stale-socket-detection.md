...
- ホスト再起動後で `${XDG_RUNTIME_DIR}` ベースなら消えているはずだが、
  `${XDG_CACHE_HOME:-$HOME/.cache}/hyoui` 配下に残骸が残っている
...
3. `hyoui list --prune-stale` 未対応バージョンの場合は手動 `unlink`:
   ```bash
   rm -- "$XDG_RUNTIME_DIR/hyoui/<session>.sock"  # or ${XDG_CACHE_HOME:-$HOME/.cache}/hyoui/<session>.sock
   ```
...
- [[DR-0006]] §socket-placement — `${XDG_RUNTIME_DIR}/hyoui/<session>.sock`
  または `${XDG_CACHE_HOME:-$HOME/.cache}/hyoui` フォールバック
